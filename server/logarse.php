...
while ($row = mysqli_fetch_array($result)) {
	...
	if ($mensagem)
	{
			$msgarray = array('msg' => $mensagem, 'nome' => 0 , 'senha' => 0, 'email' => 0, 'sexo' => 0, 'user' =>0);

	}
	else{
		
		$msgarray = array('msg' => 'Login realizado com sucesso!', 'nome' => $row['nome'], 'email' => $row['email'], 'sexo' => $row['sexo'], 'senha' => $row['senha'], 'user' => $row['id']);
	}
